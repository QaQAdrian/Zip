...
public class Zip {
    ...
    public class func unzipFile(_ zipFilePath: URL,
                                ...
                                progress: ((_ progress: Double) -> Void)? = nil, fileOutputHandler: ((_ unzippedFile: URL) -> Void)? = nil) throws {
        // File manager
    ...
    /**
     ...
     - notes: Supports implicit progress composition
     */
    public class func zipData(archiveFiles: [ArchiveFile], zipFilePath: URL, password: String?, compression: ZipCompression = .DefaultCompression, progress: ((_ progress: Double) -> Void)?) throws {
        let destinationPath = zipFilePath.path
    ...
    public class func openAndHasPassword(zip: URL) -> (Bool, Bool) {
        let zip = unzOpen64(zip.path)
        defer {
            unzClose(zip)
        }
